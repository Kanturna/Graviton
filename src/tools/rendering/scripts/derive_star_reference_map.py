...
def _build_reference_map(source_path: Path, output_path: Path, size: int) -> None:
    ...
    cx = (size - 1) * 0.5
    cy = (size - 1) * 0.5
    radius_px = size * 0.455
    feather = size * 0.035

    for y in range(size):
        for x in range(size):
            dist = math.hypot(x - cx, y - cy)
            if dist >= radius_px + feather:
                continue
            alpha = 1.0
            if dist > radius_px:
                alpha = max(0.0, 1.0 - (dist - radius_px) / max(feather, 1.0))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Derive a reusable color star reference map from a solar image."
    )
