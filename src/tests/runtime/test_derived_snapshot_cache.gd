...
class BiosphereStub:
	...
	func describe_body(id: StringName) -> Dictionary:
		...
		return {
			...
			"has_biosphere_basis": id != &"genesis",
			"dominant_track_id": 0,
			"biosphere_stage": 1,
			"dominant_potential_class": 2,
		}
...
class BiosphereScaleStub:
	...
	func describe_body(id: StringName) -> Dictionary:
		...
		return {
			"body_id": id,
			"is_supported_body_kind": id != &"genesis",
			"has_biosphere_scale_basis": id != &"genesis",
			"dominant_track_id": 0,
			"biosphere_stage": 3,
			"dominant_potential_class": 2,
			"dominant_biomass_index": 0.42,
		}


class OrbitReadoutStub:
	extends Node
	...
	func describe_body(id: StringName) -> Dictionary:
		...
		return {
			"body_id": id,
			"has_rotation_basis": id != &"genesis",
			"rotation_period_s": 86400.0 if id != &"genesis" else 0.0,
			"has_orbital_period_basis": id != &"genesis",
			"orbital_period_s": 365.25 * 86400.0 if id != &"genesis" else 0.0,
		}


class NativeSpeciesStub:
	extends Node
	...
	func describe_body(id: StringName) -> Dictionary:
		...
		return {
			"body_id": id,
			"is_supported_body_kind": id != &"genesis",
			"has_native_species_basis": id == &"planet_a" or id == &"moon_a",
			"species_complexity_class": 0,
			"species_richness_class": 1,
			"habitat_class": 0,
			"metabolism_class": 0,
			"mobility_class": 1,
		}
...
class GeneticSpeciesStub:
	...
	func describe_body(id: StringName) -> Dictionary:
		...
		return {
			"body_id": id,
			"is_supported_body_kind": id != &"genesis",
			"has_genetic_species_basis": id == &"planet_a" or id == &"moon_a",
			"lifeform_profiles": [],
...
class PopulationEstimateStub:
	...
	func describe_body(id: StringName) -> Dictionary:
		...
		return {
			"body_id": id,
			"is_supported_body_kind": id != &"genesis",
			"has_population_estimate_basis": id == &"planet_a" or id == &"moon_a",
...
static func _test_cache_tracks_interest_and_dirty_updates(ctx) -> void:
	...
	var biosphere_service := BiosphereStub.new()
	var biosphere_scale_service := BiosphereScaleStub.new()
	...
	var population_estimate_service := PopulationEstimateStub.new()
	...
	ctx.assert_true(cache.get_focus_biosphere_desc().get("body_id", &"") == &"planet_a", "configure baut auch den Proto-Biosphaeren-Focus-Snapshot")
	ctx.assert_true(cache.get_focus_biosphere_scale_desc().get("body_id", &"") == &"planet_a", "configure baut auch den Life-v2-Focus-Snapshot")
	...
	ctx.assert_true(cache.get_focus_population_estimate_desc().get("body_id", &"") == &"planet_a", "configure baut auch den Population-Estimate-Focus-Snapshot")
	...
	ctx.assert_true(cache.get_biosphere_desc(&"moon_a").get("body_id", &"") == &"moon_a", "interessierter Mond bekommt auch einen Proto-Biosphaeren-Snapshot")
	ctx.assert_true(cache.get_biosphere_scale_desc(&"moon_a").get("body_id", &"") == &"moon_a", "interessierter Mond bekommt auch einen Life-v2-Snapshot")
	...
	ctx.assert_true(cache.get_population_estimate_desc(&"moon_a").get("body_id", &"") == &"moon_a", "interessierter Mond bekommt auch einen Population-Estimate-Snapshot")
