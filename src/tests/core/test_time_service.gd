...
static func _test_last_sim_dt_tracks_authoritative_tick(ctx) -> void:
	...
	TimeService._physics_process(0.0)
	ctx.assert_true(TimeService.last_physics_process_us >= 0, "last_physics_process_us misst den Physics-Callback read-only")
	ctx.assert_true(TimeService.physics_process_total_us >= TimeService.last_physics_process_us, "physics_process_total_us akkumuliert Physics-Callback-Zeit read-only")
	TimeService.reset()
	...
	ctx.assert_true(TimeService.last_physics_process_us == 0, "reset loescht last_physics_process_us")
	ctx.assert_true(TimeService.physics_process_total_us == 0, "reset loescht physics_process_total_us")
